from flow_decomposition.utils.models import LinearModel, NonlinearModel, AutoencoderModel
...
class FlowAutoencoder:
    ...
    def __init__(self, input_dim, proj_dim, 
        ...
        """
        Initializes the FlowAutoencoder model.
        ...
        self.model = AutoencoderModel(input_dim=input_dim, proj_dim=proj_dim, n_comp=1, device=device, random_state=random_state)
    ...
    def fit(self, X, Y,
        ...
        for epoch in range(num_epochs):
            ...
            ccm_loss, vae_loss = self._compute_loss_from_dataloader(
                dataloader, num_rand_samples, method, theta, nbrs_num, exclusion_rad
            )
            
            h_norm_val = self.__compute_h_norm()
            total_loss = torch.log(ccm_loss + beta * h_norm_val + vae_loss)
    ...
    def transform(self, X, device="cpu"):
        ...
        with torch.no_grad():
            ...
            outputs = self.model.to(device)(inputs)[1][:,:,0] 
    ...
    def _compute_loss_from_dataloader(self, dataloader, num_rand_samples, method, theta, nbrs_num, exclusion_rad):
        ...
        for subset_idx, sample_idx, subset_X, subset_y, sample_X, sample_y in dataloader:
            ...
            subset_X_z, subset_X_mu, subset_X_logvar = self.model(subset_X)
            sample_X_z, sample_X_mu, sample_X_logvar = self.model(sample_X)
            ...
            subset_X_mu = subset_X_mu.reshape(subset_X_mu.size(0), subset_X_mu.size(1), -1, subset_X_mu.size(4))
            subset_X_logvar = subset_X_logvar.reshape(subset_X_logvar.size(0), subset_X_logvar.size(1), -1, subset_X_logvar.size(4))
            sample_X_mu = sample_X_mu.reshape(sample_X_mu.size(0), sample_X_mu.size(1), -1, sample_X_mu.size(4))
            sample_X_logvar = sample_X_logvar.reshape(sample_X_logvar.size(0), sample_X_logvar.size(1), -1, sample_X_logvar.size(4))
            
            # Compute the CCM-based loss for this batch.
            loss, vae_loss = self.__compute_loss(
                subset_X_mu, subset_X_logvar,
                sample_X_mu, sample_X_logvar,
            ...
            vae_loss /= num_rand_samples
            total_loss += loss.sum()
            total_vae_loss += vae_loss.sum()
        
        return total_loss, total_vae_loss
    
    def __compute_loss(self, subset_X_mu, subset_X_logvar,
                        sample_X_mu, sample_X_logvar, 
                        subset_idx, sample_idx, sample_X, sample_y, subset_X, subset_y,
        ...
        if self.subtract_autocorr:
            ...
            subset_KLD = -0.5 * torch.mean(1 + subset_X_logvar - subset_X_mu.pow(2) - subset_X_logvar.exp())
            sample_KLD = -0.5 * torch.mean(1 + sample_X_logvar - sample_X_mu.pow(2) - sample_X_logvar.exp())
            return score, (subset_KLD + sample_KLD) / 2
        else:
